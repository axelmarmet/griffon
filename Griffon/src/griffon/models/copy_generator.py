...
# LIFTED FROM https://raw.githubusercontent.com/OpenNMT/OpenNMT-py/master/onmt/modules/copy_generator.py
...
class RefactorCopyGenerator(nn.Module):
    """An implementation of pointer-generator networks
    :cite:`DBLP:journals/corr/SeeLM17`.
    ...
    * :math:`p_{copy}` the probility of copying a particular word.
    ...
    def forward(self, hidden, attn, src_map):
        ...
        # CHECKS
        tlen, batch, _ = hidden.shape
        batch_, tlen_, slen = attn.shape
        slen_, batch__, _ = src_map.shape

        assert tlen == tlen_
        assert batch == batch_ and batch_ == batch__
        assert slen == slen_
        ...
        mul_attn = torch.mul(attn, p_copy)
        #---Added------
        a = mul_attn.view(-1, batch, slen).transpose(0, 1)
        b = src_map.transpose(0, 1)
        res = torch.bmm(a, b)
        #--------------
        copy_prob = torch.bmm(
            mul_attn.view(-1, batch, slen).transpose(0, 1),
